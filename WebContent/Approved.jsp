...
<body>
	...
	%>
	<form action="Approved.jsp method="post">
		<table align="center" border="0">
			<tr>
			<tr class="EVEN">
				<td><input type="hidden" name="EmpId"
					value="<%=request.getParameter("EmpId")%>"> Emp_Id</td>
				...
				<td>
					...
					%> <select id="mytext" name="strid" style="width: 275px">
						...
						<%
							while (rs.next()) {
									if (rs.getString(2).equals(session.getAttribute("EmpId"))) {
						%>
	...
	<%
		...
		if ((request.getParameter("Emp_Id") != "NULL")) {
			try {
				...
				CallableStatement preparedStmt1 = conn.prepareCall("{call  sp_retail_store_maint_update(?,?,?)}");
				...
				String sql = "SELECT Transaction_Id,Team_Memebr,support_ticket_status,team_group_member,Ticket_Type,Ticket_Type_Support,Store_Id,Support_Priority,Comment,LastUpdate,date FROM retail_store_maint_front WHERE Ticket_Id ='"
						+ str + "'";
				//execute query
	...
	<%
		try {
				...
				ResultSet r2 = stmt.executeQuery(
						"SELECT Transaction_Id,Team_Member,Support_Priority,support_ticket_status,Comment,LastUpdate,date FROM retail_store_maint_front");
	%>
